...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text("Trying to put me against a Itachi Master huh?")
        elif user_id in DEV_USERS:
            message.reply_text("I can't act against our own akatsuke group.")
        elif user_id in DRAGONS:
            message.reply_text(
                "Fighting this Ninja here will put user lives at risk."
            )
        elif user_id in DEMONS:
            message.reply_text(
                "Bring an order from itachi Master Servant to fight a Assasin servant."
            )
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from itachi Master Servant to fight a Lancer servant."
            )
